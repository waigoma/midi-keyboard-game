...
namespace DemoMPTK
{
    public class TestMidiStream : MonoBehaviour
    {
        ...
        private float Slider(string title, float val, float min, float max, bool alignright = false, float wiLab = 100, float wiSlider = 100, float wiValue = 30)
        {
            float ret;
            GUILayout.BeginHorizontal();
            GUILayout.Label(title, alignright ? myStyle.LabelRight : myStyle.LabelLeft, GUILayout.Width(wiLab), GUILayout.Height(25));
            GUILayout.Label(Math.Round(val, 2).ToString(), myStyle.LabelRight, GUILayout.Width(wiValue), GUILayout.Height(25));
